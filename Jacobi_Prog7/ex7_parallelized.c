...
//	printf("Enter matrix size, max_iterations and max tolerance: ");
	...
	double restrict *T,  *T_new; 
	//adding restrict keyword informs OPENACC compiler that we are using pointer to matrix/data structure 
	double *temp = NULL;
...
//	print_arr(T_new,n2); printf("\n");
	...
	for(int x=1; x<=max_iter && max_diff>tol; x++) //iterate max_iter times or until max_diff<=tolerance
		...
		//divide matrix into smaller blocks of 32x32
		for(int i=1; i<=n; i++)
			{
			for(int j=1; j<=n; j++)
				{
